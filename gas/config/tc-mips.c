...
mips_parse_argument_token (char *s, char float_format)
{
  ...
  if (mips_parse_register (&s, &regno1, &channels))
      ...
      if (*s == '[')
	{
      ...
      else if ((mips_opts.arch == CPU_RSP) && *s == '.')
	{
	  /* Check for N64 RSP homebrew vector index.  */
	  expressionS element;
	  char qual;
			 ...
			 const struct mips_operand *operand)
{
  ...
  if ((mips_opts.arch == CPU_RSP) && arg->token->type == OT_CHAR)
    {
      /* RSP homebrew syntax index for load/store instructions.  */
      const char * insn_name = arg->insn->insn_mo->name;

      switch (TOLOWER (arg->token->u.ch))
	{
	case 'e':
	  /* For ltv/stv, the elementwise selector acts like the index for
	     computational instructions.  */
	  mul = (insn_name[0] != '\0' && insn_name[1] == 't') ? 2 : 1;
	  break;
	case 'b':
	  /* Some vector load/stores (lpv, ltv, luv + equivalent stores)
	     do not support byte indexing.  */
	  if (insn_name[0] != '\0' &&
	      (insn_name[1] == 'p' ||
	       insn_name[1] == 't' ||
	       insn_name[1] == 'u'))
	...
	case 'h':
	  set_insn_error_ss
	    (0, _("%s does not support .%s broadcast modifier"),
	     insn_name, (arg->token->u.ch == 'q') ? "q" : "h");
	  /* Fallthrough.  */
	default:
	  return false;
	}
...
static bool
match_reg_rsp_homebrew_index_operand (struct mips_arg_info *arg,
				      const struct mips_operand *operand,
				      unsigned int uval)
{
			     ...
			     const struct mips_operand *operand)
{
  ...
  /* May be homebrew syntax.  */
  if (arg->token->type == OT_CHAR && arg->token->u.ch != ',')
    return match_reg_rsp_homebrew_index_operand (arg, operand, uval);
		   ...
		   bfd_reloc_code_real_type *reloc_type)
{
  ...
  else if ((mips_opts.arch == CPU_RSP) && mips_optimize == 2
